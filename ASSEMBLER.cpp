...
vector<string>final_instructions;
...
bool convert_to_hexa_code(){
	for(int i = 0, j = 1; i < final_instructions.size(); ++i, ++j){
		if(final_instructions[i].size() != 14){
			cout << "Something went wrong !!" << endl;
			return false;
		}
    	hexa_code += binary_to_hexadecimal(final_instructions[i]) + '\n';
    }
    return true;
}
...
bool complete_instruction(instruction& I){
	...
	if(I.words[0] == "JUMP" or I.words[0] == "EQL" or I.words[0] == "NEQL" or I.words[0] == "INPT" or I.words[0] == "OUT") {
		if(I.words[0] == "JUMP"){
			...
			final_instructions.push_back(I.binary_code);
			return true;
		}
			...
			int dis = jumps[I.words[3]] - I.instruction_no - 1;
			if(dis < 0 or 7 < dis){
				additional += 2;
				instruction I_t1 = I;
				instruction I_t2 = I;
				I.binary_code += int_to_binary(1);
				I_t1.binary_code = "";
				I_t1.binary_code = codes["EQL"] + registors["ZERO"] + registors["ZERO"] + int_to_binary(1);
				
				for(auto& it : jumps){
					if(it.second > I.instruction_no){
						it.second += 2;
					}
				}
				for(auto& it : instruction_store){
					if(it.instruction_no > I.instruction_no){
						it.instruction_no += 2;
					}
				}
				I_t2.binary_code = codes["JUMP"] + int_to_binary_9bit(jumps[I.words[3]]);
				final_instructions.push_back(I.binary_code);
				final_instructions.push_back(I_t1.binary_code);
				final_instructions.push_back(I_t2.binary_code);
...
void stdout_details(){
	...
	cout << "Additional instructions: " << additional << endl;
